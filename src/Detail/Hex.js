...
class BaseHex extends React.Component {
    ...
    render() {
        return (
            ...
            <OverlayTrigger
                placement="top"
                overlay={<Tooltip>{this.state.tip || "double click to copy."}</Tooltip>}
            >
                    <code className={this.styling} ref={(r) => this.code = r}>{this.props.value} </code>
            </OverlayTrigger>

        )
    }
    componentDidMount() {
        this.code.addEventListener('dblclick',
            this.copy.bind(this)
        )
    }
}
