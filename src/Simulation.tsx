...
export class SimulationController extends React.Component<{
    ...
    constructor(props: any) {
        ...
        this.min_blocks = Math.round((new Date().getFullYear() - 2008) * 144 * 365 - 144 * 365 / 2);
        ...
        this.state = {
            ...
            first_tx_block: (this.max_blocks - this.min_blocks)*this.first_tx_block + this.min_blocks,
            current_block: (this.max_blocks-this.min_blocks)*this.current_block + this.min_blocks,
    ...
    changeHandler(from: Field, e: FormEvent) {
        ...
        const [[first_tx_time_secs, current_time_secs], [first_tx_block, current_block]] = this.recompute_times()
    ...
    recompute_times(): [[number, number], [number, number]] {
        ...
        const current_block = Math.round(block_delta * this.current_block + this.min_blocks);
        const first_tx_block = Math.round(block_delta * this.first_tx_block + this.min_blocks);
        return [[first_tx_time / 1000, current_time / 1000], [first_tx_block, current_block]];

    }
    delayedUpdate() {
        const [[first_tx_time, current_time], [first_tx_block, current_block]] = this.recompute_times()
        const unreachable = this.props.contract.reachable_at_time(current_time, current_block, first_tx_time, first_tx_block);
    ...
    handleSubmit(e: SimAction) {
        switch (e) {
            ...
            case "snap-time": {
                ...
                const [[first_tx_time, current_time], _x] = this.recompute_times()
                if (first_tx_time == current_time) return;

                let new_start = Math.min(first_tx_time, current_time);
                // at least one day...
                let new_end = Math.max(first_tx_time, current_time, new_start + 24 * 60 * 60);
                let delta = Math.abs(current_time - first_tx_time);
                new_start -= delta;
                new_end += delta;

                this.current_time = current_time > first_tx_time ? 0.66 : 0.33;
                this.first_tx_time = current_time > first_tx_time ? 0.33 : 0.66;
            ...
            case "snap-blocks": {
                ...
                const [_x, [first_tx_block, current_block]] = this.recompute_times()
                if (first_tx_block == current_block) return;

                let new_start = Math.min(first_tx_block, current_block);
                // at least one day...
                let new_end = Math.max(first_tx_block, current_block, new_start + 144);
                let delta = Math.abs(current_block - first_tx_block);
                new_start -= delta;
                new_end += delta;

                this.current_block = current_block > first_tx_block ? 0.66 : 0.33;
                this.first_tx_block = current_block > first_tx_block ? 0.33 : 0.66;
